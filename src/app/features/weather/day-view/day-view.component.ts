...
export class DayView extends KillZombies() {
  ...
  constructor(
    ...
    this.conditions.icon = this.sanitizer.bypassSecurityTrustUrl(
    ...
    this.ngrxstore
      ...
      .subscribe((state) => {
        if (state.unit) {
          this.unitLabel = WEATHER_TEMPRATURE_SYMBOLS[
    ...
    let unitSub = this.ngrxstore
      ...
      .subscribe((state) => {
        ...
        if (state.zip) {
          this.weatherService
            .getCurrentWeatherForZip(state.zip, { units: state.unit })
          ...
          this.weatherService
            .getCurrentWeatherForLocation(this.conditions.coord, {
              units: state.unit,
            })
            .subscribe((updatedConditions: WeatherReport) => {
              this.conditions = Object.assign({}, updatedConditions);
