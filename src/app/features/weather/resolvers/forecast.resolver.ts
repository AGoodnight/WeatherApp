...
export class ForecastResolver implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot) {
    ...
    return new Observable<ForecastReport>((obs) => {
      this.ngrxstore
        .pipe(select(myCurrentWeatherSettingsSelector))
        .subscribe((state) => {
              ...
              this.weatherService
                .getLocationForecast(
                  ...
                  },
                  {
                    ...
                    limit: WEATHER_FEATURE_DEFAULTS.DEFAULT_FORECAST_RANGE,
                  }
                )
                .pipe(take(1))
                .subscribe((forecast: ForecastReport) => {
                  obs.next(forecast);
                  obs.complete();
                });
            });
