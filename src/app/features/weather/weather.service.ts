...
export class WeatherService {
  ...
  public getCurrentWeatherForLocation(
    coordinates: GeoCoordinates,
    params: Dictionary = { units: WEATHER_FEATURE_DEFAULTS.DEFAULT_UNITS }
  ): Observable<any> {
    return new Observable<WeatherReport>((obs) => {
      this.http
        .get<WeatherResponse>(WEATHER_ENDPOINTS.CURRENT_WEATHER, {
        ...
        .subscribe((weather) => {
          this.getWeatherIcon(weather.weather[0].icon)
            .pipe(take(1))
            .subscribe((icon) => {
              let _units: string =
                (params.units as string) ||
                (WEATHER_FEATURE_DEFAULTS.DEFAULT_UNITS as string);

              let _interpretedData: WeatherReportExtras = Object.assign(
                {},
                parseUnixDate(weather.dt, _units),
                {
                  icon: icon,
                  units: _units,
                }
              ...
              obs.next(
                Object.assign({}, weather, _interpretedData) as WeatherReport
              );
              obs.complete();
            });
        });
  ...
  public getCurrentWeatherForZip(
    zip: string,
    params: Dictionary = { units: WEATHER_FEATURE_DEFAULTS.DEFAULT_UNITS }
  ): Observable<any> {
    return new Observable<WeatherReport>((obs) => {
  ...
  public getHistoricalWeatherForLocation(
    coordinates: GeoCoordinates,
    params: Dictionary = {
      units: 'imperial',
      date: '1984-04-01',
    ...
    delete _params.date;
    return this.http
      .get<WeatherResponse>(WEATHER_ENDPOINTS.HISTORICAL_WEATHER, {
  ...
  public getLocationForecast(
    ...
    return new Observable((obs) => {
      this.http
        .get<ForecastResponse>(WEATHER_ENDPOINTS.FORECAST, {
          params: Object.assign(<HttpParams>{}, params, {
            lat: String(coordinates.lat),
            lon: String(coordinates.lon),
            appid: String(API_KEY),
        ...
        .subscribe((forecast) => {
          let forecastClone: ForecastResponse = Object.assign(
            {},
            forecast
          ) as ForecastResponse;
          ...
          forecastReport.units =
            (params.units as string) ||
            (WEATHER_FEATURE_DEFAULTS.DEFAULT_UNITS as string);

          let iconsForConditions = forecastClone.list.map(
  ...
  public getWeatherIcon(iconName: string): Observable<any> {
    return new Observable((obs) => {
      this.http
        .get(BLOB_ENDPOINTS.ICON + '/' + iconName + '.png', {
          responseType: 'blob',
        })
        .pipe(take(1))
